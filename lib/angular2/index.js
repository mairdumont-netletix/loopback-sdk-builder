...
module.exports = function generate(ctx) {
  ...
  // Include or Exclude Models from generation
  if (ctx.includeModels.length > 0) {
    try {
      var includeRegExp = new RegExp(ctx.includeModels);
      var modelNames = Object.keys(ctx.models);
      var include = modelNames.filter(modelName => includeRegExp.test(modelName));
      var exclude = _.difference(modelNames, include);
      exclude.forEach(modelName => {
        if (ctx.models[modelName]) {
          delete ctx.models[modelName];
          console.log('Excluding %s from being added to SDK (by model-include)', modelName);
        }
      });
    } catch (e) {
      console.error('Error while includeModels', e);
    }
  }
  if (ctx.excludeModels.length > 0) {
    try {
      var excludeRegExp = new RegExp(ctx.excludeModels);
      var exclude = Object.keys(ctx.models).filter(modelName => excludeRegExp.test(modelName));
      exclude.forEach(modelName => {
        if (ctx.models[modelName]) {
          delete ctx.models[modelName];
          console.log('Excluding %s from being added to SDK (by model-exclude)', modelName);
        }
      });
    } catch (e) {
      console.error('Error while excludeModels', e);
    }
  }
