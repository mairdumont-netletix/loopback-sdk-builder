...
module.exports = function generate(ctx) {
  ...
  if (ctx.includeModels.length > 0) {
    var include = ctx.includeModels.split(",");
    var exclude = _.difference(Object.keys(ctx.models), include);
    exclude.forEach(modelName => {
      if (ctx.models[modelName]) {
          delete ctx.models[modelName];
          console.log('Excluding %s from being added to SDK ', modelName);
      }
    });
  }
  if (ctx.excludeModels.length > 0) {
    var exclude = ctx.excludeModels.split(",");
    exclude.forEach(modelName => {
      if (ctx.models[modelName]) {
          delete ctx.models[modelName];
          console.log('Excluding %s from being added to SDK ', modelName);
